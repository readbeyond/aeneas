...
* Contact: [aeneas AT readbeyond DOT it](mailto:aeneas@readbeyond.it)
...
or raw CSV/TXT/XML for further processing.
...
2. Python 2.7.x
3. Python optional modules `BeautifulSoup`, `numpy`, and `scikits` (`pip install ...`)
4. `ffmpeg` and `ffprobe` executables installed (`apt-get install ffmpeg*`)
5. `espeak` executable installed installed (`apt-get install espeak*`)
...
a SasS version of this package,
